...
3. Проверить в шапке ветку — по умолчанию подставляется та, из которой публикуется сайт
   (берётся из настроек Pages); если выбрать другую, рядом появится красное предупреждение.
   При необходимости выбрать ветку — ту же, из которой публикуется сайт (Settings → Pages → Build and deployment → Branch).
